...
public class ForensicAnalysis {
    ...
    */
    public Profile createSingleProfile() {
        ...
        Profile profile = new Profile(arrSTR);
    ...
    public void insertPerson(String name, Profile newProfile) {
        ...
        if (treeRoot == null){
            ...
            if (name.compareTo(prevNode.getName()) < 0){
                prevNode.setLeft(treeNode); 
            }
            else{
                prevNode.setRight(treeNode);
            }  
    ...
    private void flagProfilesOfInterest(TreeNode nodeBST){
        ...
        if (count >= Math.ceil((double)arr.length/2)){
    ...
    private TreeNode removePerson(TreeNode nodeBST, String fullName){
        ...
        if (fullName.compareTo(nodeBST.getName()) < 0){
            nodeBST.setLeft(removePerson(nodeBST.getLeft(), fullName));
        }
        else if (fullName.compareTo(nodeBST.getName()) > 0) {
            nodeBST.setRight(removePerson(nodeBST.getRight(), fullName));
        }
        else{ 
            if (nodeBST.getLeft() == null){
                return nodeBST.getRight();
            }
            if (nodeBST.getRight() == null){
                return nodeBST.getLeft();
            }
            TreeNode ptr = nodeBST; 
            TreeNode minNode = ptr.getRight();
            while (minNode.getLeft() != null){
                minNode = minNode.getLeft();
            }
            nodeBST = minNode;
            nodeBST.setRight(deleteMin(ptr.getRight()));
            nodeBST.setLeft(ptr.getLeft());
        }
        return nodeBST;
    }

    private TreeNode deleteMin(TreeNode nodeBST){
        if (nodeBST == null){
            return null;
        }
        if (nodeBST.getLeft() == null){
        ...
        nodeBST.setLeft(deleteMin(nodeBST.getLeft()));
        return nodeBST;
    }
